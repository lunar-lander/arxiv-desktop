...
export function usePDFContent() {
  ...
  const extractPDFContent = useCallback(
    async (paper, options = {}) => {
      ...
      try {
        ...
        let content;
        try {
          content = await PDFExtractionService.extractText(
            paper.localPath,
            defaultOptions
          );
        } catch (localError) {
          console.warn(
            `Local PDF extraction failed for ${paper.title}, trying URL:`,
            localError.message
          );
          if (paper.pdfUrl) {
            content = await PDFExtractionService.extractText(
        ...
        setExtractedContent((prev) =>
          new Map(prev).set(paperId, extractedData)
        );
  ...
  const extractPDFSummary = useCallback(async (paper) => {
    ...
    try {
      const summary = await PDFExtractionService.extractSummary(
        paper.localPath
      );
  ...
  const extractMultiplePDFs = useCallback(
  ...
  const getPDFContent = useCallback(
    (paperId) => {
  ...
  const formatForLLMContext = useCallback(
    (papers) => {
      let context = "";
      ...
      papers.forEach((paper, index) => {
        ...
        if (pdfContent && !pdfContent.error) {
          ...
          const authorInfo = paper.authors
          ...
          const remainingLength =
            maxTotalLength - totalContentLength - paperHeader.length;
          ...
          if (remainingLength > 1000) {
            ...
            if (contentToAdd.length > remainingLength) {
              contentToAdd =
                contentToAdd.substring(0, remainingLength - 100) +
  ...
  const getExtractionStats = useCallback(() => {
    const completed = Array.from(extractionStatus.values()).filter(
      (status) => status === "completed"
    ).length;
    const extracting = Array.from(extractionStatus.values()).filter(
      (status) => status === "extracting"
    ).length;
    const errors = Array.from(extractionStatus.values()).filter(
      (status) => status === "error"
    ).length;
